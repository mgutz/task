...
const debug = require('debug')('task-tail')
...
class Tail extends events.EventEmitter {
   ...
   *
   */
  constructor(filename, separator, options) {
    super()
    ...
    this.readBlock = self.readBlock.bind(this)
    this.buffer = ''
  ...
  var next = function() {
    if (block.type == 'close') {
      try {
        ...
        console.error('FAILED TO CLOSE FD:', err, err.stack)
      }
